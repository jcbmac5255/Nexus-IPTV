...
import com.nexus.iptv.data.local.dao.CombinedM3uProfileDao
import com.nexus.iptv.data.local.dao.CombinedM3uProfileMemberDao
import com.nexus.iptv.data.local.dao.ProviderDao
import com.nexus.iptv.data.local.entity.CombinedM3uProfileEntity
import com.nexus.iptv.data.local.entity.CombinedM3uProfileMemberEntity
...
import com.nexus.iptv.domain.model.ActiveLiveSourceOption
            ...
            if (memberProviders.isEmpty()) {
                ...
                combine(categoryFlows) { arrays ->
                    arrays.toList()
                        .map { it as Triple<*, *, *> }
                        .flatMap { triple ->
                            val member = triple.first as com.nexus.iptv.domain.model.CombinedM3uProfileMember
                            val categories = triple.second as List<Category>
                            val hiddenCategoryIds = triple.third as Set<Long>
                            categories
                                .filter {
                                    !it.isVirtual &&
                                        it.id != com.nexus.iptv.domain.repository.ChannelRepository.ALL_CHANNELS_ID &&
