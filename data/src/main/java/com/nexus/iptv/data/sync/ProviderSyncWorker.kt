...
internal suspend fun reconcileTargetedProviderStatus(
    ...
    provider: com.nexus.iptv.data.local.entity.ProviderEntity,
    result: com.nexus.iptv.domain.model.Result<Unit>,
    ...
    when (result) {
        ...
        is com.nexus.iptv.domain.model.Result.Loading -> Unit
    }
}
...
class ProviderSyncWorker(
    ...
    private suspend fun syncXtreamProviderIfStale(
        ...
        provider: com.nexus.iptv.data.local.entity.ProviderEntity
    ): com.nexus.iptv.domain.model.Result<Unit> {
        ...
        if (liveStale) {
            when (val liveResult = entryPoint.syncManager().retrySection(
                ...
                is com.nexus.iptv.domain.model.Result.Error -> return liveResult
                else -> Unit
            }
        }
        if (epgStale) {
            when (val epgResult = entryPoint.syncManager().syncEpg(provider.id, force = false)) {
                is com.nexus.iptv.domain.model.Result.Error -> return epgResult
                else -> Unit
            }
        }
        if (movieIndexDue) {
