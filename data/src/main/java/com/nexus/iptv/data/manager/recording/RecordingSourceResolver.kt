...
import com.nexus.iptv.data.remote.xtream.ResolvedStreamUrl
...
class RecordingSourceResolver @Inject constructor(
    ...
    suspend fun resolveLiveSource(
        ...
        val providerLabel = providerDao.getById(providerId)?.let { provider ->
            when {
                provider.type == com.nexus.iptv.domain.model.ProviderType.XTREAM_CODES -> "${provider.name} • Xtream"
                provider.type == com.nexus.iptv.domain.model.ProviderType.M3U -> "${provider.name} • M3U"
                provider.type == com.nexus.iptv.domain.model.ProviderType.STALKER_PORTAL -> "${provider.name} • Stalker"
                else -> provider.name
            }
        }
