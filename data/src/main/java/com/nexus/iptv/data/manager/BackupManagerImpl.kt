package com.nexus.iptv.data.manager
...
import com.nexus.iptv.data.local.dao.PlaybackHistoryDao
import com.nexus.iptv.data.local.dao.ProviderDao
import com.nexus.iptv.data.local.dao.RecordingScheduleDao
import com.nexus.iptv.data.local.dao.VirtualGroupDao
import com.nexus.iptv.data.local.entity.ProviderEntity
import com.nexus.iptv.data.local.entity.RecordingScheduleEntity
import com.nexus.iptv.data.mapper.toDomain
import com.nexus.iptv.data.mapper.toEntity
import com.nexus.iptv.data.preferences.ParentalPinBackupData
import com.nexus.iptv.data.preferences.PreferencesRepository
import com.nexus.iptv.data.security.CredentialCrypto
import com.nexus.iptv.domain.manager.BackupData
import com.nexus.iptv.domain.manager.BackupConflictStrategy
import com.nexus.iptv.domain.manager.BackupImportPlan
import com.nexus.iptv.domain.manager.BackupImportResult
import com.nexus.iptv.domain.manager.BackupManager
import com.nexus.iptv.domain.manager.BackupPreview
import com.nexus.iptv.domain.manager.RecordingScheduleImportDisposition
import com.nexus.iptv.domain.manager.RecordingScheduleImportOutcome
import com.nexus.iptv.domain.manager.RecordingScheduleImportSummary
import com.nexus.iptv.domain.manager.ProtectedCategoryBackup
import com.nexus.iptv.domain.manager.RecordingManager
import com.nexus.iptv.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): com.nexus.iptv.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext com.nexus.iptv.domain.model.Result.error("Failed to open output stream")

            com.nexus.iptv.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            com.nexus.iptv.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): com.nexus.iptv.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext com.nexus.iptv.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext com.nexus.iptv.domain.model.Result.error("Unsupported backup version")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext com.nexus.iptv.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            com.nexus.iptv.domain.model.Result.success(
                BackupImportResult(
            ...
            com.nexus.iptv.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["playerDecoderMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val decoderMode = com.nexus.iptv.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = com.nexus.iptv.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = com.nexus.iptv.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<com.nexus.iptv.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.nexus.iptv.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<com.nexus.iptv.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.nexus.iptv.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun com.nexus.iptv.domain.model.RecordingItem.toScheduledRecordingBackup(
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<com.nexus.iptv.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun com.nexus.iptv.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<com.nexus.iptv.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<com.nexus.iptv.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<com.nexus.iptv.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<com.nexus.iptv.domain.model.PlaybackHistory>>() {}.type
