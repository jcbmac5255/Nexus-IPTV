...
import com.nexus.iptv.data.local.dao.ChannelStageCategorySummary
...
import com.nexus.iptv.data.remote.stalker.StalkerCategoryRecord
import com.nexus.iptv.data.remote.stalker.StalkerItemRecord
import com.nexus.iptv.data.remote.stalker.StalkerProgramRecord
import com.nexus.iptv.data.remote.stalker.StalkerProviderProfile
import com.nexus.iptv.data.remote.stalker.StalkerSession
...
class SyncManagerTest {
    ...
    private class FakeSyncMetadataRepository : com.nexus.iptv.domain.repository.SyncMetadataRepository {
    ...
    fun `sync_xtream_staged_live_commit_preserves_hidden_categories_and_channels`() = runTest {
        ...
        assertThat(result.isSuccess).isTrue()
        val insertedCategories = argumentCaptor<List<com.nexus.iptv.data.local.entity.CategoryImportStageEntity>>()
        val insertedChannels = argumentCaptor<List<com.nexus.iptv.data.local.entity.ChannelImportStageEntity>>()
    ...
    fun `retrySection_movies_queues_xtream_index_categories_only`() = runTest {
        ...
        assertThat(metadata?.movieSyncMode).isEqualTo(com.nexus.iptv.domain.model.VodSyncMode.UNKNOWN)
        assertThat(xtreamBackend.requestedActions).contains("get_vod_categories")
        assertThat(xtreamBackend.requestedActions).doesNotContain("get_vod_streams")
        val jobs = argumentCaptor<XtreamIndexJobEntity>()
        verify(xtreamIndexJobDao, atLeastOnce()).upsert(jobs.capture())
    ...
    fun sync_stalker_persists_only_vod_and_series_categories_during_initial_sync() = runTest {
        ...
        val metadata = syncMetadataRepo.getMetadata(1L)
        assertThat(metadata?.movieSyncMode).isEqualTo(com.nexus.iptv.domain.model.VodSyncMode.LAZY_BY_CATEGORY)
    ...
    fun sync_stalker_recovers_when_live_categories_fail_but_bulk_channels_work() = runTest {
        ...
        assertThat(result).isInstanceOf(Result.Success::class.java)
        val categoryStagesCaptor = argumentCaptor<List<com.nexus.iptv.data.local.entity.CategoryImportStageEntity>>()
        verify(catalogSyncDao, atLeastOnce()).insertCategoryStages(categoryStagesCaptor.capture())
        assertThat(categoryStagesCaptor.allValues.flatten().any { it.type == com.nexus.iptv.domain.model.ContentType.LIVE && it.name == "News" }).isTrue()
    ...
    fun sync_stalker_upfront_epg_without_xmltv_imports_native_portal_guide() = runTest {
        ...
        assertThat(result).isInstanceOf(Result.Success::class.java)
        val insertedProgramsCaptor = argumentCaptor<List<com.nexus.iptv.data.local.entity.ProgramEntity>>()
 ...
 */
private suspend fun com.nexus.iptv.data.remote.stalker.StalkerApiService.stubStreamBulkEpg(
    programs: List<com.nexus.iptv.data.remote.stalker.StalkerProgramRecord>,
    periodHours: Int = 6
) {
    org.mockito.kotlin.whenever(
        streamBulkEpg(
        ...
        val cb = invocation.arguments[3] as suspend (com.nexus.iptv.data.remote.stalker.StalkerProgramRecord) -> Unit
        programs.forEach { cb(it) }
        com.nexus.iptv.domain.model.Result.success(programs.size)
    }
}

/** Stubs the streaming live-channel path. Emits [items] through the captured callback. */
private suspend fun com.nexus.iptv.data.remote.stalker.StalkerApiService.stubStreamLiveStreams(
    items: List<com.nexus.iptv.data.remote.stalker.StalkerItemRecord>
) {
    org.mockito.kotlin.whenever(
        streamLiveStreams(
            org.mockito.kotlin.any(),
        ...
        @Suppress("UNCHECKED_CAST")
        val cb = invocation.arguments[2] as suspend (com.nexus.iptv.data.remote.stalker.StalkerItemRecord) -> Unit
        items.forEach { cb(it) }
        com.nexus.iptv.domain.model.Result.success(items.size)
    }
}

/** Stubs the streaming live-channel path to return an error result without emitting items. */
private suspend fun com.nexus.iptv.data.remote.stalker.StalkerApiService.stubStreamLiveStreamsError(message: String) {
    org.mockito.kotlin.whenever(
        streamLiveStreams(
            org.mockito.kotlin.any(),
            org.mockito.kotlin.any(),
    ...
    ).doSuspendableAnswer { com.nexus.iptv.domain.model.Result.error(message) }
}

/** Counterpart to [stubStreamBulkEpg] for the per-channel streamed EPG path. */
private suspend fun com.nexus.iptv.data.remote.stalker.StalkerApiService.stubStreamEpg(
    channelId: String,
    programs: List<com.nexus.iptv.data.remote.stalker.StalkerProgramRecord>,
    periodHours: Int = 6
) {
    org.mockito.kotlin.whenever(
        ...
        @Suppress("UNCHECKED_CAST")
        val cb = invocation.arguments[4] as suspend (com.nexus.iptv.data.remote.stalker.StalkerProgramRecord) -> Unit
        programs.forEach { cb(it) }
        com.nexus.iptv.domain.model.Result.success(programs.size)
    }
}
