...
class MarkAsWatchedTest {
    ...
    private class FakePlaybackHistoryRepository : PlaybackHistoryRepository {
        ...
        override suspend fun markAsWatched(history: PlaybackHistory): com.nexus.iptv.domain.model.Result<Unit> {
            lastMarkedHistory = history
            return com.nexus.iptv.domain.model.Result.success(Unit)
        }
        override suspend fun recordPlayback(history: PlaybackHistory) = com.nexus.iptv.domain.model.Result.success(Unit)
        override suspend fun updateResumePosition(history: PlaybackHistory) = com.nexus.iptv.domain.model.Result.success(Unit)
        override suspend fun flushPendingProgress() = com.nexus.iptv.domain.model.Result.success(Unit)
        override suspend fun removeFromHistory(contentId: Long, contentType: ContentType, providerId: Long) = com.nexus.iptv.domain.model.Result.success(Unit)
        override suspend fun clearAllHistory() = com.nexus.iptv.domain.model.Result.success(Unit)
        override suspend fun clearHistoryForProvider(providerId: Long) = com.nexus.iptv.domain.model.Result.success(Unit)
        override suspend fun clearLiveHistoryForProvider(providerId: Long) = com.nexus.iptv.domain.model.Result.success(Unit)
    }
}
